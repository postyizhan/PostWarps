...
class ItemBuilder(private val plugin: PostWarps) {
    ...
    fun createWarpItem(
        ...
        // 使用地标的显示材质创建物品
        val item = createWarpItemWithMaterial(mainConfig, iconConfig, player, data, warp) ?: return null
     ...
     * 使用地标材质创建物品
     */
    private fun createWarpItemWithMaterial(
        mainConfig: ConfigurationSection,
        iconConfig: IconConfig?,
        player: Player,
        data: Map<String, Any>,
        ...
        // 优先使用地标的显示材质，如果是头颅则使用头颅信息
        val item = if (warp.isPlayerHead()) {
            com.github.postyizhan.util.SkullUtil.createSkullFromInfo(warp.getSkullInfo())
        } else {
            // 使用普通材质
            try {
                ItemStack(Material.valueOf(warp.displayMaterial.uppercase()))
            } catch (e: IllegalArgumentException) {
                plugin.logger.warning("未知的材料类型: ${warp.displayMaterial}")
